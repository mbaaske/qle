  ...
  if(type){
    f = qlm->intern_qfValue();
	  ...
	  double *typf=qfs->typf,
	  		 *score=qlm->score,
	  		 *qscore=qlm->qlsolve.score;

	  f=0.0;
	  for (int i=0;i<n;++i){
	 	 qscore[i] = score[i]*typf[i];							// scaling components of quasi-score vector
	 	 f += qscore[i] * qscore[i];							// only if using norm^2 of quasi-score
	  }
	  if(!R_FINITE(f)){
...
void fnGrad(qfs_options qfs, double *g, double *d, int type, int &info){
	...
	if(type){
		MEMCPY(g,qlm->score,n);							// simply use quasi-score as the gradient
		MEMCPY(d,g,n);									// copy score for computation of Newton direction
	} else {
	  int i=0;
	  double *typf=qfs->typf,
	         *qscore=qlm->qlsolve.score;
	  ...
	  for(i=0;i<n;++i)
	    qscore[i] = -qscore[i]*typf[i];
      matmult(qlm->qimat,n,n,qscore,n,1,g,info);
	  if(info > 0){
...
SEXP QSopt(SEXP R_start, SEXP R_qsd, SEXP R_qlopts, SEXP R_X, SEXP R_Vmat, SEXP R_cm, SEXP R_opt) {
    ...
    /* scoring iteration */
    double fval=HUGE_VAL, qnorm=HUGE_VAL;
	...
	if(fntype){
		...
		for (int i=0;i<xdim;++i)
		 qnorm += SQR(score[i]*typf[i]);						// scaling components of quasi-score vector
		if(!R_FINITE(qnorm)){
...
void backtr(int n, double *xold, double &fold,  double *d, double *g, double *x,
  ...
  int i=0;
  double s=1.0, tmp=0.0, test=0.0, dirlen=0.0,
		 stepmin=TOLSTEP, alpha=1e-3, *typx=qfs->typx;
  ...
  slope=(fntype ? -fold : innerProduct(g,d,n));     // slope equals monitor function for quasi-deviance (fntype==1)
  ...
  else WRR("Line search: relative step length should not be zero.")
  ...
  for(;;) {
	  ...
	  fnQS(x,qfs,f,fntype,bounds,info);
	  if(info > 0) break;
	  /* set some required decrease */
	  tmp=alpha*s*slope;
	  if(f <= fold + tmp) 			  				// should be < 0
	  	return;
	  if(s < stepmin){
		  f=fold;
		  for(i=0;i<n;++i)
...
//  if(test > 0.0)
//   stepmin=MAX(qfs->xtol_rel/test,TOLSTEP);
//  else WRR("stepmin in line search should not bee zero.")
//
//  for(;;) {
//	  delta=s;
//	  for (i=0;i<n;++i)
...
//	  fnQS(x,qfs,f,fntype,bounds,info);
//	  if(info > 0) break;
//	  /* set required decrease (heuristically) */
//	  tmp=alpha*s*slope;
//	  if(f <= fold + tmp) 			  					// should be < 0
//	  	return;
//	  else if(s < stepmin){
...
//			  fnQS(x,qfs,f,fntype,bounds,info);			// and compute things a last time
//			  return;
//		  } else {
//			 /* switch monitor function and restart */
//			 fntype = (fntype > 0 ? 0 : 1);					// type of monitor function
//			 fnQS(xold,qfs,fold,fntype,bounds,info);  	// re-compute monitor at fold
//			 if(info > 0) break;
//			 fnGrad(qfs,g,fntype,info);					// gradient at fold (quasi-score for monitor quasi-deviance)
//			 if(info > 0) break;
